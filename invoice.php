      ...
      <div class="row">
		  ...
		  <div class="col-lg-10">
				...
				<?php
					...
					$id = $_GET['id'];
					$pel = mysql_query("SELECT * FROM pelanggan WHERE email='$user'");
                  	$cek = mysql_fetch_array($pel);
                  	$pem = mysql_query("SELECT pembayaran.*,orders.id_pelanggan FROM pembayaran,orders WHERE pembayaran.id_order=orders.id_order AND orders.id_order='$id'");
                  	$cekpem = mysql_fetch_array($pem);
					//var_dump($user);die;
				?>
				<div class="col-md-9">
					<h2>PT. Asri Celebes Mandiri</h2>
					<h4><?=$id?></h4>
				</div>
				<div class="col-md-3">
					<br>
				...
				if ($cekpem['id_order'] != $id) {
				     	header('location:myorder.php');
				}elseif ($cek['email'] != $_SESSION['username']) {
				     	header('location:myorder.php');
				}
				?>
		  	  </div>
